...
class Portfolio:
    def __init__(self, initial_capital: float, log_trades: bool = True):
...
def run_backtest(
    ...
    portfolio = Portfolio(INITIAL_CAPITAL, log_trades=verbose)
    ...
    for idx, current_date in enumerate(all_dates):
        # 先执行挂单
        future_orders = []
        known_prices_for_sizing = dict(last_close_prices)
        ...
        for order in pending_orders:
            ...
            if order.action == "BUY":
                ...
                known_prices_for_sizing[code] = price
                target_value = portfolio.total_value(known_prices_for_sizing) * target_pct
                bought = portfolio.buy(order.execute_dt, code, name, market, price, target_value, order.reason, atr)
                if bought:
                    known_prices_for_sizing[code] = price
            else:
                known_prices_for_sizing[code] = price
        ...
        for code in list(portfolio.positions.keys()):
            ...
            if ENABLE_INTRADAY_EXIT:
                ...
                if intraday_exit and not any(order.code == code and order.action == "SELL" for order in pending_orders):
                    ...
                    if exit_dt:
                        pending_orders.append(PendingOrder(exit_dt[0], code, "SELL", intraday_exit[1]))
                        continue

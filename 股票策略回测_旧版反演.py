...
def infer_output_dir() -> Path:
    ...
    default_root = PROJECT_ROOT / "outputs" / "股票策略回测_旧版反演"
    ...
    if "stock_splits" in parts:
        ...
        if split_idx + 1 < len(parts):
            ...
            return PROJECT_ROOT / "outputs" / "股票策略回测_旧版反演" / split_name
    return default_root
...
def add_daily_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    float_cols = [
        "open", "high", "low", "close", "volume", "amount", "pre_close", "ma20", "ma60",
        "ma20_slope_5", "ma60_slope_5", "ret20", "atr14", "amount_ma20",
    ]
...
def select_candidates(current_date: pd.Timestamp, daily_data: dict[str, pd.DataFrame], instruments: pd.DataFrame, st_status: pd.DataFrame, cooldown_until: dict[str, pd.Timestamp]) -> list[tuple[str, float]]:
    candidates = []
...
def find_intraday_entry(intraday_df: pd.DataFrame, day: pd.Timestamp) -> pd.Timestamp | None:
    ...
    for i in range(2, len(day_df)):
        ...
        row = day_df.iloc[i]
        if ((prev2 is not None and pd.isna(prev2["ma3"])) or pd.isna(prev_row["ma3"]) or pd.isna(prev_row["ma8"]) or pd.isna(row["ma3"]) or pd.isna(row["ma8"])):
            continue
        pullback = bool((prev2 is not None and prev2["close"] <= prev2["ma3"] * 1.004) or prev_row["close"] <= prev_row["ma3"] * 1.004)
        ...
        breakout = bool(row["close"] > row["ma3"] >= row["ma8"] and row["close"] >= prev_row["high"] * 0.998 and row["close"] > day_open * 0.998 and row["bar_ret"] > 0.001 and close_from_low >= MIN_INTRADAY_CLOSE_FROM_LOW_PCT and day_chase <= MAX_INTRADAY_CHASE_FROM_DAY_OPEN_PCT)
...
def should_exit_early_weakness(pos: dict, holding_days: int) -> bool:
    ...
    return bool(latest_ret <= EARLY_WEAK_EXIT_MIN_CLOSE_RET and max_high_ret < EARLY_WEAK_EXIT_MIN_HIGH_RET)
...
def build_trade_diagnostics(sells_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    reason_summary = (sells_df.groupby("reason", dropna=False).agg(交易数=("profit", "count"), 总盈亏=("profit", "sum"), 平均盈亏=("profit", "mean"), 胜率=("profit", lambda s: (s > 0).mean() * 100)).sort_values("总盈亏").round(2))
    holding_df = sells_df.copy()
    holding_df["hold_days"] = (pd.to_datetime(holding_df["datetime"]).dt.normalize() - pd.to_datetime(holding_df["entry_dt"]).dt.normalize()).dt.days
    holding_df["持有区间"] = pd.cut(holding_df["hold_days"], bins=[-1, 2, 4, 7, 10, 16, 999], labels=["0-2", "3-4", "5-7", "8-10", "11-16", "17+"])
    holding_summary = (holding_df.groupby("持有区间", observed=False).agg(交易数=("profit", "count"), 总盈亏=("profit", "sum"), 平均盈亏=("profit", "mean"), 胜率=("profit", lambda s: (s > 0).mean() * 100)).round(2))
    return reason_summary, holding_summary


def export_backtest_outputs(daily_df: pd.DataFrame, trades_df: pd.DataFrame, sells_df: pd.DataFrame, metrics: dict[str, float]) -> tuple[Path, Path, Path, Path, Path]:
...
def run_backtest(verbose: bool = True, export_outputs: bool = True, preloaded_context: dict | None = None):
        ...
        for code in list(portfolio.positions.keys()):
            ...
            if holding_days >= MAX_HOLDING_DAYS:
                ...
                if exit_dt:
                    ...
                    continue
            trend_break = bool(holding_days >= MIN_HOLDING_DAYS_BEFORE_TREND_EXIT and (daily_row["close"] < daily_row["ma20"] * 0.992 or (prev_daily_row is not None and prev_daily_row["close"] < prev_daily_row["ma20"] and daily_row["close"] < daily_row["ma20"])))
            market_exit = bool(risk_off and holding_days >= MIN_HOLDING_DAYS_BEFORE_TREND_EXIT and daily_row["close"] < daily_row["ma20"])
    ...
    if export_outputs:
        equity_path, trades_path, summary_path, reason_summary_path, holding_summary_path = export_backtest_outputs(daily_df, trades_df, sells_df, metrics)
    else:
        equity_path = trades_path = summary_path = reason_summary_path = holding_summary_path = None
